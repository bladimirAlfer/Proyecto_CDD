...
# ==============================================================================
# 1. ARQUITECTURA DEL MODELO (GCN + LSTM)
# ==============================================================================
...
if not kpi_df.empty:
    ...
    delta_total = ((total_proj - total_real_ant) / total_real_ant) * 100 if total_real_ant > 0 else 0.0
...
k1.metric("Año de Proyección", f"{future_year}")
k2.metric(f"Proyección Total {kpi_label_suffix}", f"{total_proj:,.0f}", f"{delta_total:.1f}%")
k3.metric("Zona de Mayor Riesgo", top_dist_name)
k4.metric(f"Incidentes en {top_dist_name}", f"{top_dist_val:,.0f}")
...
with tab1:
    ...
    if map_data.empty:
        ...
        view_state = pdk.ViewState(latitude=map_data["lat"].mean(), longitude=map_data["lon"].mean(), zoom=10, pitch=map_pitch)
        tooltip_html = {"html": "<b>{Distrito}</b><br/>🚨 Proyección: <b>{txt_proy}</b><br/>⚠️ Riesgo: {Nivel_Riesgo}<br/>🔙 Anterior: {txt_ant}<br/>📈 {Tendencia}", "style": {"backgroundColor": "#1f2937", "color": "white", "zIndex": "1000"}}
        st.pydeck_chart(pdk.Deck(map_style="dark", initial_view_state=view_state, layers=[layer], tooltip=tooltip_html), use_container_width=True)
...
with tab2:
    ...
    df_long = pd.melt(chart_data, id_vars=["Distrito"], value_vars=["Real_Anterior", "Proyeccion"], var_name="Escenario", value_name="Incidentes")
